...
# Hugging Face's serverless Inference API can cold-start a model on first
# use (can take 20-30s) -- give it more headroom than a typical REST call.
HUGGINGFACE_TIMEOUT = httpx.Timeout(30.0, connect=10.0)
...
async def create_http_clients(config: Config) -> HTTPClients:
    ...
    huggingface = httpx.AsyncClient(
        ...
        headers={"Authorization": f"Bearer {config.huggingface_api_key or ''}"},
    ...
    return HTTPClients(groq=groq, tts=tts, huggingface=huggingface, kite=kite, tavily=tavily)


async def close_http_clients(clients: HTTPClients) -> None:
    await clients.groq.aclose()
    await clients.tts.aclose()
    await clients.huggingface.aclose()
